...
export class EthListen {
  constructor(api, action, address, blockProvider = undefined) {
    this.api = api
    this.action = action
    ...
    const ticker = async () => {
      try {
        ...
        for (const item of data.result) {
          ...
          if (this.action === 'txlist' && confirmationsTotal > 0) {
          ...
          } else if (this.action === 'txlistinternal') {
            if (this.transferReceivedHashs[item.transactionHash] === undefined) {
              this.transferReceivedHashs[item.transactionHash] = true
              callbacks && callbacks.onReceived && callbacks.onReceived(item)
            }
            if (
              this.transferConfirmationedHashs[item.transactionHash] === undefined &&
              item.index >= 1 && item.type === 'call' && item.value > 0
            ) {
              this.transferConfirmationedHashs[item.transactionHash] = true
              callbacks &&
                callbacks.onConfirmation &&
                callbacks.onConfirmation(item)
            }
